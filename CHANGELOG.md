...
### Fixes
- 消息导航 rail 标记去掉 hover 淡背景：0.1.32 给命中区加了一层
  `rgba(255,255,255,0.07)` 的 hover 背景提示可点范围，用户反馈太突兀
  （"背后有一个淡淡的背景……删掉"）；命中区本身保持透明，点击范围不变
  （整轨宽 ×16px），只保留白条自身的 hover 反馈（24→30px 变亮）

## [0.1.33] - 2026-08-16

### Features
- 消息导航 rail 悬停**预览浮层**：鼠标悬停在标记白条上时，白条右侧浮出
  一个深色圆角卡片（330px 宽、深背景+边框+阴影、带底部淡出提示文字被截
  断），显示该条用户消息的完整内容（markdown 渲染，行内代码 `xxx` 样式
  与聊天区一致），替代原来的浏览器原生 title 单行提示；浮层垂直居中于
  所悬停的白条，超高时 JS 钳制在聊天区内不越窗；离开白条有 250ms 宽限期
  ——2px 白条难以瞄准，宽限期让鼠标能从白条跨过间隙移到浮层上阅读，浮层
  内按住可选中文本（指针按下期间不隐藏）；键盘 Tab 聚焦白条同样弹出浮层
  （role=tooltip）
- spike/probe-message-nav.mjs 用真实鼠标移动验证：悬停白条 → 浮层出现且
  内容为该条消息、浮层在 rail 右侧 ≥8px、宽 ≥280px；鼠标移上浮层不关闭；
  移开后 450ms 内隐藏（原生 title 断言移除，26 断言）
